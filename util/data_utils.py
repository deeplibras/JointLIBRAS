...
def image_preloader(target_path, image_shape, crop=(0,0,0,0), mode='file', normalize=True,
    ...
    n_classes = np.max(labels) + 1
    X = ImagePreloader(images, image_shape, crop, normalize, grayscale)
...
class ImagePreloader(Preloader):
    def __init__(self, array, image_shape, crop=(0,0,0,0), normalize=True, grayscale=False):
        fn = lambda x: self.preload(x, image_shape, crop, normalize, grayscale)
        super(ImagePreloader, self).__init__(array, fn)

    def preload(self, path, image_shape, crop=(0,0,0,0), normalize=True, grayscale=False):
        img = load_image(path)
